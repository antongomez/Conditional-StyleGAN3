...
def load_dataset_and_split_info(input_dir, output_dir, filename, split_format, read_raw_data=True):
    ...
    return {"dataset": dataset, "split_info": split_info}
        ...
        for image_batch, label_batch in dataloader:
            ...
            if actual_batch_size < batch_size:
                ...
                pad_images = torch.zeros(
                    (pad_size, *image_batch.shape[1:]), dtype=image_batch.dtype, device=image_batch.device
                )
                image_batch = torch.cat([image_batch, pad_images], dim=0)

                pad_labels = torch.zeros(
                    (pad_size, *label_batch.shape[1:]), dtype=label_batch.dtype, device=label_batch.device
                )
...
def generate_pixel_classification_map(
    predictions,
    test_indices,
    centers,
    segmentation_data,
...
def calculate_pixel_accuracy_optimized(
    ...
    # 4. Calculate metrics
    OA, AA, class_accuracies = calculate_metrics_vectorized(pixel_output, truth, num_classes, label_map, show_progress)
...
def calculate_pixel_accuracy_ultra_optimized(
    ...
    test_indices = split_info["test_indices"]
    train_val_indices = np.concatenate([split_info["train_indices"], split_info["validation_indices"]])
    num_classes = split_info["split_stats"]["num_classes"]
    ...
    class_accuracies = np.divide(
        class_corrects, class_totals, out=np.zeros_like(class_corrects, dtype=float), where=class_totals != 0
    )
