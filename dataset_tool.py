...
def is_image_ext(fname: Union[str, Path]) -> bool:
    ...
    return f".{ext}" in PIL.Image.EXTENSION or ext == "npy"  # type: ignore
...
def make_transform(
    ...
    def scale(width, height, img):
        ...
        img = PIL.Image.fromarray(img) if channels < 5 else img
        ww = width if width is not None else w
        hh = height if height is not None else h
        img = img.resize((ww, hh), PIL.Image.LANCZOS) if channels < 5 else img  # We are not applying resize
        return np.array(img) if channels < 5 else img
...
def convert_dataset(
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        idx_str = f"{idx:08d}"
        channels = img.shape[2] if img.ndim == 3 else 1
        archive_fname = f"{idx_str[:5]}/img{idx_str}.{'png' if channels in [1, 3] else 'npy'}"
